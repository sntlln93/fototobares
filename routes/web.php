...
// Etapas de producción por producto y sus consumos de insumos: solo gerencia
Route::middleware(['auth', 'role:master,administración'])->group(function () {
    ...
    Route::post('/production-statuses/{productionStatus}/stockables', [ProductionStatusStockableController::class, 'store'])->name('production-statuses.stockables.store');
    Route::delete('/production-statuses/{productionStatus}/stockables/{stockable}', [ProductionStatusStockableController::class, 'destroy'])->name('production-statuses.stockables.destroy');
});
